...
var dientes = <?php echo json_encode($dientes); ?>;
console.log(antecendetes, dientes)
</script>
<div class="container">
		...
		<svg height="50" class="[[i.tipoDiente]]" width="50"  ng-repeat="i in adultoAbajo" id="[[i.id]]">
  			...
  			<text x="20" y="8" fill="navy" stroke="navy" value="[[i.value]]" stroke-width="0.1" style="font-size: 10pt;font-weight:normal">[[i.value]]</text>
					...
					<div class="row row_border ">
							...
							<input type="input" name="diente_presentes" id="diente_presentes" style="color:black ">	
...
for (var i = 0; i < dientes.length; i++) {
			...
			if (datos[i].estado_diente == 3 || datos[i].estado_diente ==  8) {
				$('#'+datos[i].valor_diente +' circle').each(function(){
					if (saltarEachCirculo == false) {
						pintar(datos[i].estado_diente,this);
						saltarEachCirculo = true;
					}
				});
			}
			else if (datos[i].estado_diente == 4 || datos[i].estado_diente ==  6) 
			{
				$('#'+datos[i].valor_diente +' polygon').each(function(){
			...
			else
			{
				$('#'+datos[i].valor_diente +' polygon').each(function(){
					...
					if (datos[i].cara_diente == $(this).attr('value')) {
						pintar(datos[i].estado_diente,this);
					}
				});	
			}
...
$.each(antecendetes, function(i, val) {
        ...
        $('#diente_presentes').val(val.diente_presentes);
        
        $('#id_enfermedad').val(val.id_control_placa);
        $('#fecha').val(val.fecha);
...
function insertar_historia(){
    ...
    onSuccess : function($form) {
    	var cantidad = $('#diente_presentes').val();
...
{	
  ...
  {		
  	...
  	$('svg').each(function(){
		...
		var sreg=formulario.elements.diente_presentes.value*4;
